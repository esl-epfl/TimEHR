...
from sklearn.metrics import accuracy_score,roc_auc_score, f1_score, average_precision_score, roc_curve,precision_recall_curve, precision_score, recall_score, confusion_matrix
...
def compute_utility(X_train_real, X_train_fake, X_test_real, X_test_fake, y_train_real, y_train_fake, y_test_real, y_test_fake):
    ...
    if "TRTR" in UTILITY_MODE:
        ...
        # fit real model
        model_real = lgb.LGBMClassifier(random_state=42,scale_pos_weight=scale_pos_weight, verbosity=1 )
        ...
        metrics.update({'TRTR/AUROC': roc_auc_score(y_true, y_score),
    ...
    if "TSTR" in UTILITY_MODE:
        ...
        print("\n[info] TSTR")
        model_fake = lgb.LGBMClassifier(random_state=42,scale_pos_weight=scale_pos_weight, verbosity=1 )
        ...
        metrics.update({'TSTR/AUROC': roc_auc_score(y_true, y_score),
...
def compute_synthcity(X_real, X_fake, X_test):
    ...
    X_ref_syn = X_syn
    X_augmented = None
    selected_metrics={ 'privacy': ['delta-presence', 'k-anonymization', 'k-map', 'distinct l-diversity', 'identifiability_score', 'DomiasMIA_BNAF', 'DomiasMIA_KDE', 'DomiasMIA_prior']}
    selected_metrics={
    ...
    metrics_syn_test = {('Synthcity[Train-Test]/'+k):v for k,v in metrics_syn_test.items()}
    ...
    metrics_syn = {('Synthcity[Train-Synthetic]/'+k):v for k,v in metrics_syn.items()}
...
def compute_nnaa(REAL, FAKE, TEST):
    ...
    def ff(A,B, self=False):
        ...
        a = np.sum(A ** 2, axis=1).reshape(A.shape[0], 1) + np.sum(B.T ** 2, axis=0).reshape(1, B.shape[0])
        b = np.dot(A, B.T) * 2
        distance_matrix = a - b
    ...
    aa_train = (np.sum(distance_TS > distance_TT)/distance_TT.shape[0] + np.sum(distance_ST > distance_SS)/distance_SS.shape[0])/ 2

    aa_test = (np.sum(distance_ES > distance_EE)/distance_EE.shape[0] + np.sum(distance_SE > distance_SS)/distance_SS.shape[0]) / 2

    aa_train_test = (np.sum(distance_TE > distance_TT)/distance_TT.shape[0] + np.sum(distance_ET > distance_EE)/distance_EE.shape[0])/ 2
    ...
    metrics_asym= {
        "Train-Fake": np.sum(distance_TS > distance_TT)/distance_TT.shape[0] - distance_TT.shape[0]/(distance_TT.shape[0]+distance_SS.shape[0]),
        "Fake-Train": np.sum(distance_ST > distance_SS)/distance_SS.shape[0]- distance_SS.shape[0]/(distance_TT.shape[0]+distance_SS.shape[0]),
        "Test-Fake": np.sum(distance_ES > distance_EE)/distance_EE.shape[0]- distance_EE.shape[0]/(distance_EE.shape[0]+distance_SS.shape[0]),
        "Fake-Test": np.sum(distance_SE > distance_SS)/distance_SS.shape[0]- distance_SS.shape[0]/(distance_EE.shape[0]+distance_SS.shape[0]),
        "Train-Test": np.sum(distance_TE > distance_TT)/distance_TT.shape[0]- distance_TT.shape[0]/(distance_TT.shape[0]+distance_EE.shape[0]),
        "Test-Train": np.sum(distance_ET > distance_EE)/distance_EE.shape[0]- distance_EE.shape[0]/(distance_TT.shape[0]+distance_EE.shape[0]),
    }
...
def compute_mia_knn(REAL,FAKE,TEST):
    ...
    metrics = {
        ...
        "MIA/knn-auroc": roc_auc_score(np.concatenate([np.ones_like(train_nearest_dist), np.zeros_like(test_nearest_dist)], axis=0), np.concatenate([train_nearest_dist, test_nearest_dist], axis=0)),
    }
...
def evaluate(inputs, wandb_task_name='DEBUG',config={}):
    ...
    X_test_by_train, y_test = xgboost_embeddings(df_ts_test, state_vars, df_base = df_ts_train)
    ...
    save_examples(torch.from_numpy(inputs['train_data'][:10]),
                   torch.from_numpy(inputs['fake_data'][:10]),
                     n_ts=len(state_vars), epoch_no=-1)
    ...
    if DATASET in ['p12','p19','mimic']:
        metrics = compute_utility(X_train_real, X_train_fake, X_test_real, X_test_fake,\
            y_train_real, y_train_fake, y_test_real, y_test_fake)
        wandb.log(metrics)
    ...
    if DATASET in ['p12','p19','mimic-big']:
        ...
        CORR_METHOD = 'ffill'
    metric = compute_temp_corr(df_train_real, df_train_fake, df_test, state_vars, corr_method=CORR_METHOD, corr_th=CORR_TH)
    print(metric)
    wandb.log(metric)
