...
class CWGAN(nn.Module):
    def __init__(self, opt, d_static=1):
        ...
        print(f"Generator has {sum(p.numel() for p in self.gen.parameters() if p.requires_grad):,} trainable parameters")
        print(f"Critic has {sum(p.numel() for p in self.critic.parameters() if p.requires_grad):,} trainable parameters")
        # initializate optimizer
        self.opt_gen = optim.Adam(self.gen.parameters(), lr=opt.lr, betas=(0.0, 0.9))
        self.opt_critic = optim.Adam(self.critic.parameters(), lr=opt.lr, betas=(0.0, 0.9))
    ...
    def from_pretrained(self, run_path):
        ...
        for file in run.files():
            if ".pt" in file.name:        
                _ = file.download(replace=False,exist_ok=True, root=f'.local/{run_path}')
        ...
        if run_creation_time < ref_time:
            print("run is old")
            Z_DIM, N_CHANNELS, FEATURES_GEN, FEATURES_CRITIC = run.config["Z_DIM"], run.config["CHANNELS_IMG"], run.config["FEATURES_GEN"], run.config["FEATURES_CRITIC"]
            IMG_SIZE, D_STATIC, COND, KERNEL_SIZE = run.config["IMG_SIZE"], run.config["D_STATIC"], run.config["COND"], run.config["KERNEL_SIZE"]
        else:
            Z_DIM,  N_CHANNELS, FEATURES_GEN, FEATURES_CRITIC = run.config['z_dim'], run.config['channels'], run.config['feat_gen'], run.config['feat_cri']
            IMG_SIZE, D_STATIC, COND, KERNEL_SIZE = run.config['img_size'], self.d_static, run.config['cond'], run.config['kernel_size']
            pass
        ...
        checkpoint = torch.load(f'.local/{run_path}/model_weights/critic_{last_epoch}.pt')
        self.critic = Critic(
            N_CHANNELS,
            IMG_SIZE,
            FEATURES_CRITIC,
    ...
    def eval_epoch(self, loader):
        ...
        with torch.no_grad():    
            for batch_idx, batch in enumerate(tqdm(loader,leave=False)):
                ...
                # sample noise
                noise = torch.randn(cur_batch_size, self.opt.z_dim, 1, 1).to(self.device)
        ...
        }, step=self.epoch_no,commit=False)
    ...
    def train_epoch(self, train_loader):
        ...
        tot_L1 = 0  
        tot_corr_loss = 0
        for batch_idx, batch in enumerate(tqdm(train_loader, leave=False, desc="Batches")):
            ...
            CORR_fake = torch.matmul(fake[:,0].reshape(-1,img_size).T,fake[:,0].reshape(-1,img_size))/NN
            CORR_real = torch.matmul(real[:,0].reshape(-1,img_size).T,real[:,0].reshape(-1,img_size))/NN
            ...
            loss_gen = -torch.mean(gen_fake) + L1*self.opt.lambda_l1 + CORR_LOSS*self.opt.lambda_corr
        ...
        wandb.log({"Train/loss_reg": tot_loss_reg/tot_n_samples,
    ...
    def train(self, train_dataset, val_dataset, wandb_task_name='DEBUG'):
        ...
        for epoch in tqdm(range(self.opt.epochs+1), desc="Epochs"):
            ...
            if (epoch) % 5 == 0:
                ...
                save_examples(sample_real[:10], sample_fake[:10], n_ts=N_VARS, epoch_no=self.epoch_no)
            ...
            if (epoch) % 50 == 0:
                ...
                torch.save(self.critic.state_dict(), f'./model_weights/critic_{epoch}.pt')

                # saving optimizer and scheduler
                torch.save(self.opt_gen.state_dict(), f'./model_weights/opt_gen_{epoch}.pt')
                torch.save(self.opt_critic.state_dict(), f'./model_weights/opt_critic_{epoch}.pt')
...
class Pix2Pix(nn.Module):
    def __init__(self, opt, d_static=1):
        ...
        num_params_gen = sum(p.numel() for p in self.gen.parameters() if p.requires_grad)
        num_params_disc = sum(p.numel() for p in self.disc.parameters() if p.requires_grad)
        ...
        self.opt_disc = optim.Adam(
            self.disc.parameters(), lr=opt.lr, betas=(0.5, 0.999),)
        self.opt_gen = optim.Adam(
            self.gen.parameters(), lr=opt.lr, betas=(0.5, 0.999))
    ...
    def from_pretrained(self, run_path):
        ...
        if run_creation_time < ref_time:
            print("run is old")
            N_CHANNELS, FEAT_GEN, FEAT_DISC = run.config["CHANNELS_IMG"], run.config["FEAT_GEN"], [8,16]
            IMG_SIZE, D_STATIC, COND, KERNEL_SIZE = run.config["IMG_SIZE"], run.config["D_STATIC"], run.config["COND"], run.config["KERNEL_SIZE"]
        else:
            N_CHANNELS, FEAT_GEN, FEAT_DISC = run.config['channels'], run.config['feat_gen'], run.config['feat_disc']
            IMG_SIZE, D_STATIC, COND, KERNEL_SIZE = run.config['img_size'], self.d_static, run.config['cond'], run.config['kernel_size']
            pass
    ...
    def eval_epoch(self, loader):
        ...
        for idx, batch in enumerate(loop):
            ...
            with torch.no_grad():
                ...
                CORR_fake = torch.matmul(y_fake.reshape(-1,img_size).T,y_fake.reshape(-1,img_size))/NN
                CORR_real = torch.matmul(y.reshape(-1,img_size).T,y.reshape(-1,img_size))/NN
                CORR_LOSS = nn.L1Loss()(CORR_fake,CORR_real)
    ...
    def train_epoch(self, loader):
        ...
        for idx, batch in enumerate( tqdm(loader, leave=False,  desc="Batches")):
            ...
            with torch.cuda.amp.autocast():
                ...
                # compute total loss for generator
                G_loss = G_fake_loss + L2 * self.opt.lambda_l1 + CORR_LOSS * self.opt.lambda_corr
    ...
    def train(self, train_dataset, val_dataset, wandb_task_name='DEBUG'):
        ...
        for epoch in tqdm(range(self.opt.epochs+1), desc="Epochs"):
            ...
            if (epoch) % 50 == 0:
                ...
                torch.save(self.opt_gen.state_dict(), f'./model_weights/opt_gen_{epoch}.pt')
                torch.save(self.opt_disc.state_dict(), f'./model_weights/opt_disc_{epoch}.pt')

                torch.save(self.g_scaler.state_dict(), f'./model_weights/g_scaler_{epoch}.pt')
                torch.save(self.d_scaler.state_dict(), f'./model_weights/d_scaler_{epoch}.pt')
...
class TimEHR(nn.Module):
    def __init__(self,opt):
        ...
        self.cwgan = CWGAN(opt.cwgan, d_static=opt.data.d_static)
        self.pix2pix = Pix2Pix(opt.pix2pix, d_static=opt.data.d_static)
    ...
    def generate(self, train_dataset, train_schema, count=1000, method='ctgan'):
        ...
        if method=='ctgan':
            ...
            X = pd.DataFrame(real_static, columns=train_schema.static_processor['demo_vars_enc'])
            ...
            generated_static = syn_model.generate(
                count=count, cond=conditional#  X['Label'].values # real_prevalence # gen_labels
            ).dataframe().values  # generate only patients with the outcome = 1
            print(f'{count} samples generated using CTGAN')
        else:
            logging.info(f"Using static data from the training set ({len(train_dataset)} samples)")
            real_static, real_data = self._get_data(train_dataset)
        ...
        while generated_static.shape[0]>0:
            ...
            sta_fake = torch.from_numpy(generated_static[:cur_batch_size]).to(self.device).float()
