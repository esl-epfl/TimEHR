...
class Critic(nn.Module):
    def __init__(self, 
        ...
        if conditional:
            self.embed = nn.Linear(d_static,self.image_size*self.image_size, bias=False)
            self.d_ch_cond = 1
            pass
        else:   
            self.d_ch_cond = 0
        ...
        self.disc = nn.Sequential(
            ...
            nn.Conv2d(channels_img+self.d_ch_cond, features_d, kernel_size=self.kernel_size, stride=self.stride, padding=self.padding),
            nn.LeakyReLU(0.2),
            # _block(in_channels, out_channels, self.kernel_size, self.stride, self.padding)
            self._block(features_d, features_d * 2,self.kernel_size, self.stride, self.padding),
            self._block(features_d * 2, features_d * 4,self.kernel_size, self.stride, self.padding),
            self._block(features_d * 4, features_d * 8,self.kernel_size, self.stride, self.padding),
    ...
    def __init__(self, 
        ...
        if image_size == 64:
            self.net = nn.Sequential(
                ...
                self._block(channels_noise+self.d_ch_cond, features_g * 16, 4, 1, 0),  # img: 4x4
                self._block(features_g * 16, features_g * 8, self.kernel_size, self.stride, self.padding),  # img: 8x8
                self._block(features_g * 8, features_g * 4, self.kernel_size, self.stride, self.padding),  # img: 16x16
                self._block(features_g * 4, features_g * 2, self.kernel_size, self.stride, self.padding),  # img: 32x32
                # self._block(features_g * 2, features_g, 4, 2, 1),  # img: 64x64
                nn.ConvTranspose2d(
                    features_g*2, channels_img, kernel_size=self.kernel_size, stride=self.stride, padding=self.padding
                ),
            ...
            self.net = nn.Sequential(
                ...
                self._block(features_g * 2, features_g, 4, 2, 1),  # img: 64x64
                nn.ConvTranspose2d(
                    features_g, channels_img, kernel_size=self.kernel_size, stride=self.stride, padding=self.padding
                ),
                # Output: N x channels_img x 128 x 128
                nn.Tanh(),
                # nn.Sigmoid(),
...
class Gen_pix(nn.Module):
    def __init__(self, 
        ...
        self.initial_down = nn.Sequential(
            nn.Conv2d(in_channels+self.d_ch_cond, features,
                      4, 2, 1, padding_mode="reflect"),
            nn.LeakyReLU(0.2),
            # nn.ReLU(),
        )
        self.down1 = Block(features, features * 2, down=True,
                           act="leaky", use_dropout=False)
        self.down2 = Block(
    ...
    def forward(self, x, sta=None):
        ...
        return self.final_up(torch.cat([up7, d1], 1)) # shape = (batch_size, n_ch, *256, *256)
...
def test_cwgan():
    ...
    print("number of params in gen",sum(p.numel() for p in gen.parameters() if p.requires_grad))
